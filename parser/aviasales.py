...
def get_ticket_matrix(from_city_iata:str, to_city_iata:str):
    params = {'origin': from_city_iata, 'destination': to_city_iata, 'one_way':'true'}
    print(f'Link get ticket from to: {service_link}')
    print(f'?Request params: {params}\n')
    req = requests.get(service_link, params=params)
...
def get_iata_code(from_city:str, to_city:str):
    params = {'q': f'from {from_city} to {to_city}'}
    print(f'Link get iata code: {service_iata_link}')
    print(f'?Request params: {params}\n')
    req = requests.get(service_iata_link, params=params)
    data = json.loads(req.text)
...
def get_locate_by_ip(user_ip):
    params = {'locale':'ru', 'callback':'user_location_iata', 'ip':user_ip}
    print(f'Link get location by ip: {service_get_locate_by_ip_link}')
    print(f'?Request params: {params}\n')
    req = requests.get(service_get_locate_by_ip_link, params=params)
...
service_link = 'http://min-prices.aviasales.ru/calendar_preload?'

# Адрес GET запроса для определения кода IATA по названию города
service_iata_link = 'https://www.travelpayouts.com/widgets_suggest_params?'

# Адрес GET запроса для определения местоположния по ip пользователя
service_get_locate_by_ip_link = 'http://www.travelpayouts.com/whereami?'
